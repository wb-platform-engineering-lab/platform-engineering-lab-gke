...
> **CI/CD concepts introduced:** GitHub Actions, Workload Identity Federation, Artifact Registry, Trivy, SonarCloud, pre-commit | **Builds on:** Phase 4 Helm charts

[📝 Take the quiz](https://wb-platform-engineering-lab.github.io/platform-engineering-lab-gke/phase-5-ci-cd/quiz.html)
...
sonar.sources=phase-4-helm/app
...
echo "# test" >> phase-4-helm/app/backend/app.py
...
The CD workflow commits the new image tag directly into `values.yaml`. ArgoCD (Phase 6) watches this file and syncs the cluster automatically when it changes.
...
This lab updates `values.yaml` in the same repo as the application code. The strict GitOps pattern uses two repos: one for application code (triggers CI), one for cluster config (ArgoCD watches it). CI opens a PR against the config repo with the new image tag — it never pushes directly. Implemented in Phase 6.
...
[Back to main README](../README.md) | [Next: Phase 6 — GitOps](../phase-6-gitops/README.md)
