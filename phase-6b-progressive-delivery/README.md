...
> **Progressive delivery concepts introduced:** Rollout, Canary strategy, AnalysisTemplate, AnalysisRun, automatic rollback | **Builds on:** Phase 6 GitOps with ArgoCD

[▶ Watch the incident animation](https://wb-platform-engineering-lab.github.io/platform-engineering-lab-gke/phase-6b-progressive-delivery/incident-animation.html) · [📝 Take the quiz](https://wb-platform-engineering-lab.github.io/platform-engineering-lab-gke/phase-6b-progressive-delivery/quiz.html)
...
Cluster running with Phase 6 (ArgoCD + coverline-backend deployed) and Phase 6 (Prometheus running in the `monitoring` namespace — required by the AnalysisTemplate):
...
kubectl apply -f phase-6b-progressive-delivery/analysis-template.yaml

# Remove the existing Deployment (the Rollout will manage pods instead)
kubectl delete deployment coverline-backend

# Apply the Rollout
kubectl apply -f phase-6b-progressive-delivery/rollout.yaml
```
...
Update the image tag in `phase-4-helm/charts/backend/values.yaml`, push to main, and watch the full GitOps + progressive delivery loop:
...
```bash
kubectl delete -f phase-6b-progressive-delivery/
...
[Back to main README](../README.md) | [Next: Phase 7 — Observability](../phase-7-observability/README.md)
