...
[📝 Take the quiz](https://wb-platform-engineering-lab.github.io/platform-engineering-lab-gke/phase-3-helm/quiz.html)
...
```bash
helm template coverline phase-3-helm/charts/backend/
```
...
```bash
helm install coverline phase-3-helm/charts/backend/
```

### Step 2: Install the frontend

```bash
helm install coverline-frontend phase-3-helm/charts/frontend/
```
...
```bash
helm upgrade coverline phase-3-helm/charts/backend/ --set replicaCount=3
```
...
Phase 3 adds no GCP costs. PostgreSQL and Redis run as pods on the existing GKE cluster.
...
This lab passes the PostgreSQL password via `--set auth.password=coverline123`. In production, secrets must come from Vault (Phase 7) or a secrets manager — never from a values file versioned in Git or passed on the command line where they appear in shell history.
...
[Back to main README](../README.md) | [Next: Phase 4 — CI/CD](../phase-4-ci-cd/README.md)
