...
> **GitOps concepts introduced:** ArgoCD, Application, Sync Policy, Self-Heal, Prune | **Builds on:** Phase 4 CI/CD pipeline

[📝 Take the quiz](https://wb-platform-engineering-lab.github.io/platform-engineering-lab-gke/phase-5-gitops/quiz.html)
...
ArgoCD Applications:
  coverline-backend  → phase-3-helm/charts/backend   selfHeal: true   prune: true
  coverline-frontend → phase-3-helm/charts/frontend  selfHeal: true   prune: true
...
```
phase-5-gitops/
├── argocd-app-backend.yaml    ← Application pointing to phase-3-helm/charts/backend
└── argocd-app-frontend.yaml   ← Application pointing to phase-3-helm/charts/frontend
```

ArgoCD watches `phase-3-helm/charts/` — the same charts managed by Phase 3. The CD pipeline from Phase 4 updates `values.yaml` in those charts, triggering an ArgoCD sync.
...
- Helm charts from Phase 3 deployed
- CI/CD pipeline from Phase 4 running
...
spec:
  ...
  source:
    ...
    targetRevision: main
    path: phase-3-helm/charts/backend
    helm:
      valueFiles:
        - values.yaml
  destination:
...
| `path` | `phase-3-helm/charts/backend` | The same chart managed by Phase 3 |
...
Edit `phase-3-helm/charts/backend/values.yaml` and change `replicaCount` to `3`:

```bash
sed -i 's/replicaCount: 2/replicaCount: 3/' phase-3-helm/charts/backend/values.yaml
...
```bash
sed -i 's/replicaCount: 3/replicaCount: 2/' phase-3-helm/charts/backend/values.yaml
git add phase-3-helm/charts/backend/values.yaml
...
| **Phase 5 additional cost** | **$0** |

---

## GitOps concept: reconciliation vs. imperative deploys

Phase 4 used `helm upgrade` inside a CI job — an imperative command executed once. If the job fails halfway, the cluster is in an unknown state. If two jobs run concurrently, they race.
...
[Back to main README](../README.md) | [Next: Phase 5b — Progressive Delivery](../phase-5b-progressive-delivery/README.md)
