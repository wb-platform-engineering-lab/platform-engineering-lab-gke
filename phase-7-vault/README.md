...
> **Vault concepts introduced:** KV v2, Dynamic credentials, Kubernetes auth, Vault Agent Injector, JWT auth, GCP KMS auto-unseal | **Builds on:** Phase 6 observability cluster

[📝 Take the quiz](https://wb-platform-engineering-lab.github.io/platform-engineering-lab-gke/phase-7-vault/quiz.html)
...
- Phase 3 PostgreSQL deployed (`helm install postgresql ...`)
- Phase 6 Prometheus running in the `monitoring` namespace (required by vault-monitoring)
...
PG_HOST=$NODE_IP PG_PORT=$NODE_PORT \
  ...
  bash phase-7-vault/vault-dynamic-secrets.sh
```
...
Push any change to `phase-3-helm/app/` on `main`. In the workflow run, the `Authenticate to Vault` step exchanges the GitHub OIDC token for a 15-minute Vault token and reads secrets as masked environment variables.
...
| **Phase 7 additional cost** | **~$0.69/day** |
