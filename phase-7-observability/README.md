...
> **Observability concepts introduced:** Prometheus, Grafana, Loki, Promtail, PrometheusRule, LogQL | **Builds on:** Phase 6 GitOps cluster

[▶ Watch the incident animation](https://wb-platform-engineering-lab.github.io/platform-engineering-lab-gke/phase-7-observability/incident-animation.html) · [📝 Take the quiz](https://wb-platform-engineering-lab.github.io/platform-engineering-lab-gke/phase-7-observability/quiz.html)
...
CoverLine apps running in the cluster (ArgoCD synced from Phase 6):
...
```bash
kubectl apply -f phase-7-observability/coverline-alerts.yaml
```
...
kubectl delete -f phase-7-observability/coverline-alerts.yaml
```
...
This lab measures infrastructure metrics only. In production, add `prometheus-flask-exporter` to the backend to expose HTTP request rate, latency, and error rate per endpoint — the metrics the Phase 6b AnalysisTemplate needs for a proper HTTP-level canary gate.
...
[Back to main README](../README.md) | [Next: Phase 3 — Vault](../phase-3-vault/README.md)
